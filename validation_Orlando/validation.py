"""
Hierarchical Insect Classifier Validation Script
=================================================

This script validates a trained hierarchical (multitask) classification model on a 
directory of images organized by species. It calculates metrics at three taxonomic 
levels: Family, Genus, and Species.

Directory Structure
-------------------
The validation directory should contain subdirectories named after each species, 
with images inside:
...
The subdirectory names MUST match the species names used during training.
...
Basic usage:
    ...
    python validation.py \\
      ...
      --weights /path/to/best_multitask.pt \\
      --species "Coccinella septempunctata" "Apis mellifera" "Bombus terrestris"

With custom image size (must match training):
...
----------------
    ...
    results = validate(
        ...
        validation_dir="/path/to/validation/data",
        hierarchical_weights="/path/to/best_multitask.pt",
        img_size=640
    )
...
------
The script prints:
- Taxonomy information (Family, Genus, Species mappings)
- Processing progress bar
- Detailed classification metrics at all three taxonomic levels:
...
Important Notes
---------------
- Species names must match the exact order used during training
- Image size (--img_size) should match the size used during training (default: 640)
- The script automatically retrieves taxonomy information from GBIF API
- If taxonomy is saved in the model checkpoint, it will use that instead
"""
...
def validate(species_list, validation_dir, hierarchical_weights, img_size=640, batch_size=32):
    """
    ...
    """
    validator = HierarchicalValidator(hierarchical_weights, species_list, img_size, batch_size)
...
class HierarchicalValidator:
    def __init__(self, hierarchical_model_path, species_names, img_size=640, batch_size=32):
        cuda_cleanup()
