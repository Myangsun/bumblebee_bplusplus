...
"""
    ...
    if environmental_context:
        ...
        # Default to first typical background
...
def generate_synthetic_image(species: str,
    ...
    prompt = create_chain_of_thought_prompt(
        species, variant_type, environmental_context)
...
def generate_synthetic_dataset(species: str,
    ...
    for i in range(num_images):
        ...
        print(
            f"  Generating image {i+1}/{num_images} - Context: {full_context}")

        result = generate_synthetic_image(
            species=species,
            # Use fewer refs later
...
def main():
    ...
    for species in target_species:
        generate_synthetic_dataset(
            species=species,
            num_images=1,  # Adjust based on your needs
            api_key=api_key
        )
