...
**Title:** True Absence or Detection Failure: Expert-Calibrated Filtering for Rare-Species Generative Augmentation
...
- **Pipeline (3 stages).** (1) tergite-level morphological prompts → 1,500 synthetics with 0 LLM-flagged structural failures (expert review on 150-image sample still flags 46 structural-related issues — the LLM-expert gap motivates stage 3); (2) LLM-as-judge scoring; (3) expert-calibrated BioCLIP probe (150 expert labels)
- **Headline.** At the single-split reading on the fixed test set, **D6 (expert-probe) achieves the highest macro F1 in the study at 0.840, +0.025 over the 0.815 D1 baseline**, with every filtered variant (D2–D6) exceeding baseline; D6 is also numerically the best generative-filter variant on every protocol (single-run 0.840, multi-seed 0.834, 5-fold 0.824), and is the only variant with **zero harmful subtractive-ablation cells** across the three rare species. Under multi-seed averaging the aggregate ranking tightens (D1 0.839, D6 0.834) because only 3 of 16 species are augmented and the macro-F1 lever is bounded by construction — the per-species causal ablation evidence is where the D6 advantage cleanly shows

**Speaker script (≈ 45 s)**

> Automated biodiversity monitoring turns every missed rare-species detection into a silent undercount — an absence in the record that looks no different from a species that truly isn't there. For three rare Massachusetts bumblebees with 22 to 162 training images, a ResNet-50 classifier sits at F1 between 0.50 and 0.62 while common species clear 0.85. I built a three-stage pipeline to close this gap: tergite-level anatomical prompts that eliminate structural generation failures, an LLM judge that scores morphology, and finally a BioCLIP probe trained on 150 annotations from a domain expert. At the single-split reading on the fixed test set, the expert-calibrated filter D6 achieves the highest macro F1 in the study at 0.840, twenty-five hundredths above the 0.815 baseline, with every filtered variant beating baseline; D6 is also numerically best among the generative-filter variants on every evaluation protocol. Under the tighter multi-seed averaging the aggregate ranking rearranges because only three of sixteen species are augmented and the macro-F1 lever is bounded — so the sharper evidence is per-species causal: D6 is the only variant in the study whose subtractive-ablation cells are all non-harmful, and the only variant with zero harmed B. ashtoni or B. sandersoni test images under multi-seed majority vote. The expert supervision is doing work the LLM rule and the unsupervised centroid rule do not.
...
- **Anatomy-anchored prompts.** Tergite-level colour maps drive generation; **0 / 1,500 LLM-flagged structural failures** under the judge rubric, though expert review on a 150-image stratified sample still surfaces 46 structural-related flags (impossible geometry, extra/missing limbs, wrong scale) — this LLM-vs-expert gap on residual geometry, plus a larger gap on colour fidelity, is the motivation for stage 3
...
- **Multi-protocol downstream evaluation.** Single-split, 5-seed × fixed split, and 5-fold CV paired t-tests — **D6 is numerically best on every protocol among filter variants**, and **D6 is the only variant with zero harmed rare-species test images for B. ashtoni and B. sandersoni** (multi-seed majority vote, Table 5.9)
- **Causal confirmation under subtractive ablation.** **D6 is the only variant across D3/D4/D5/D6 in which all three rare species' synthetics are collectively helpful** (recoveries −0.121 / −0.056 / −0.025, no harmful cell in the 3 × 3 matrix). D5 still carries the full D3 B. ashtoni harm (+0.182) — unsupervised centroid distance does not suffice
- **Cross-species transfer is the largest single lever.** Additive ablation: **D1 + B. sandersoni-only synthetics yields macro F1 0.857** — the highest single-split figure in the study — driven by a **+0.218 boost to B. ashtoni** rather than a self-boost; the easiest-to-generate species lifts the hardest-to-generate species' decision boundary

**Speaker script (≈ 60 s)**

> Six contributions. First, the prompt template — encoding tergite colour maps at the anatomical level drives structural generation failures to zero across 1,500 generations, isolating the remaining problem as colour fidelity. Second, the calibration study: directly comparing the LLM judge against a domain expert on 150 stratified synthetics, strict pass-fail decisions agree only seven percentage points above chance and the LLM morphology score is a near-random ranker of expert quality. Third, I train a linear probe on BioCLIP features with those 150 labels; within the same 200-image-per-species training budget the probe-selected subset contains roughly three times more expert-validated synthetics than the LLM's. Fourth, multi-protocol downstream evaluation: at the single-split reading D6 wins outright at 0.840 macro F1 (+0.025 over the 0.815 baseline) and is numerically the best generative-filter variant on every protocol — single-run, multi-seed, 5-fold — while being the only variant with zero harmed B. ashtoni and B. sandersoni test images under multi-seed majority vote. Fifth, subtractive single-species ablation causally confirms the filter: D6 is the only variant across D3/D4/D5/D6 in which dropping any rare species' synthetics LOSES F1 — every D6 synthetic pool is collectively helpful, while D5 centroid inherits the full unfiltered ashtoni harm. Sixth — the biggest surprise — additive ablation shows that training on D1 plus only B. sandersoni synthetics yields macro F1 0.857, the highest single-run number in the entire study, and the gain comes from a +0.218 boost to B. ashtoni rather than B. sandersoni itself. Cross-species knowledge transfer is the largest lever we found.
...
> I built an expert-calibrated filter for generative bumblebee augmentation that beats the standard LLM-as-judge approach by selecting three times more expert-validated synthetics within the same training budget, delivers the highest single-split macro F1 in the study at 0.840 (+0.025 over the 0.815 baseline), and is the only variant with no harmful cells in the per-species subtractive ablation.
