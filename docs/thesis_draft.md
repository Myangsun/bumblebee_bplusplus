...
The 150-label protocol exposes a calibration gap (strict-pass agreement 56 % vs. 49 % expected under independence; morph-mean AUC 0.56) between the LLM judge and the expert; a 150-label-supervised BioCLIP probe achieves LOOCV AUC strict 0.792 and selects a 200-image subset per species that is roughly 3× richer in expert-strict images than the LLM-rule filter. Downstream classification F1 under the resulting D5 (centroid) and D6 (probe) variants — reported alongside D1–D4 in Section 5.5 — translates the filter-level gap partially: under apples-to-apples seed-42 single-run comparison D6 is numerically the best generative-filter variant (0.840, tied with D4 at that macro value) but aggregate leadership goes to D2 CNP (0.845) and D1 baseline (0.843) because augmentation touches only three of sixteen species. The cleaner D6 signals are per-species: D6 is the only variant whose subtractive ablation (Table 5.7) leaves all three rare-species synthetics collectively helpful, and the only variant with zero harmed B. ashtoni and B. sandersoni test images under multi-seed majority vote (Table 5.9). The 5-fold paired t-test at df = 4 cannot detect the ≤ 0.003 D6-vs-D4 macro-F1 gap, so the downstream improvement is directional at this sample size (Section 6.5). CKNNA alignment (Huh et al., 2024) between each filter's 200-image selection and the real-image distribution in BioCLIP space is [TODO: deferred to the multi-annotator replication registered in Section 7.2.1].

### 5.5 Augmentation Method Comparison

#### 5.5.1 Single-run overview

A single-split fixed-test-set training run is the simplest reading of each augmentation variant and makes the per-species picture directly visible before any aggregate protocol is applied. Table 5.4b reports single-run macro F1 with 10,000-iteration bootstrap 95 % CI alongside per-species F1 for the three rare species, and Figure 5.18a visualises the same numbers. To keep the comparison apples-to-apples, **all six rows use the same seed-42 training run on the identical 2,362-image fixed test set** — D1–D4 have additional unseeded default runs (those numbers live in the earlier draft and in `docs/final_metrics.md` Section 1) but the unseeded run for D1 yields 0.8145 which lies near the low end of the 5-seed distribution and is not directly comparable to the D5/D6 rows (for which no unseeded run was launched). Using seed 42 uniformly equalises the sampling noise that would otherwise bias the row ordering.

**Under the fair seed-42 comparison, D2 CNP achieves the highest single-run macro F1 (0.845, 95 % CI [0.803, 0.875])**, narrowly ahead of D1 baseline (0.843) and the expert-probe filter D6 (0.840); D4 LLM-filtered (0.839), D3 unfiltered (0.830) and D5 centroid (0.824) trail. All six variants sit inside each other's 95 % CIs, so single-run macro F1 does not separate the variants statistically on its own. The per-species picture is sharper: **D4 leads on B. ashtoni (0.727, up from 0.615 at baseline)**, D6 is second on that species (0.667); **D2 leads on B. flavidus (0.788, up from 0.746)**, D6 second (0.750); D1 baseline leads on B. sandersoni (0.706). Among the three generative-filter variants (D4, D5, D6), D6 is numerically best on macro F1 and on B. flavidus, but the D4 B. ashtoni lead (0.727) shows that the LLM filter does productive work on the ashtoni species where its per-feature over-strict rate (Section 5.4.2) happens to remove the right synthetics.

*Table 5.4b: Single-run fixed-test-set summary (seed 42, f1 checkpoint). Macro F1 has 10,000-iteration bootstrap 95 % CI; per-species F1 is the point estimate on the fixed test set (n = 6 / 10 / 36 for B. ashtoni / B. sandersoni / B. flavidus). Using seed 42 uniformly across all six variants keeps the sampling noise aligned; the unseeded default runs for D1–D4 are reported in Section 1 of `docs/final_metrics.md` for reference.*

| Dataset | Macro F1 | 95 % CI | B. ashtoni F1 | B. sandersoni F1 | B. flavidus F1 |
|---|---:|---|---:|---:|---:|
| **D1 Baseline** | 0.843 | [0.805, 0.871] | 0.615 | **0.706** | 0.746 |
| **D2 CNP** | **0.845** | [0.803, 0.875] | 0.600 | 0.667 | **0.788** |
| **D3 Unfiltered** | 0.830 | [0.788, 0.863] | 0.545 | 0.571 | 0.645 |
| **D4 LLM-filtered** | 0.839 | [0.798, 0.868] | **0.727** | 0.625 | 0.725 |
...
| Dataset | Single-run macro F1 (seed 42) | 95% CI | Rare F1 (ash / sand / flav) | Acc | Multi-seed macro F1 | Rare F1 (ash / sand / flav) | Acc | 5-fold CV macro F1 | 5-fold pooled 95% CI | Rare F1 (ash / sand / flav) | Acc |
|---|---:|---|---|---:|---:|---|---:|---:|---|---|---:|
| **D1 Baseline** | 0.843 | [0.805, 0.871] | 0.615 / 0.706 / 0.746 | 0.890 | **0.839 ± 0.006** | 0.614 / 0.622 / 0.760 | 0.890 | 0.832 ± 0.013 | [0.818, 0.846] | 0.621 / 0.466 / 0.747 | 0.892 |
| **D2 CNP** | **0.845** | [0.803, 0.875] | 0.600 / 0.667 / 0.788 | 0.892 | 0.822 ± 0.014 | 0.577 / 0.477 / 0.724 | 0.886 | **0.837 ± 0.013** | [0.823, 0.850] | 0.685 / 0.433 / **0.806** | 0.891 |
| **D3 Unfiltered synthetic** | 0.830 | [0.788, 0.863] | 0.545 / 0.571 / 0.645 | 0.892 | 0.828 ± 0.009 | 0.608 / 0.494 / 0.669 | 0.891 | 0.820 ± 0.024 | [0.807, 0.835] | 0.576 / 0.326 / 0.764 | 0.889 |
| **D4 LLM-filtered** | 0.839 | [0.798, 0.868] | 0.727 / 0.625 / 0.725 | 0.886 | 0.831 ± 0.008 | 0.609 / 0.533 / 0.709 | 0.890 | 0.821 ± 0.019 | [0.808, 0.836] | 0.577 / 0.398 / 0.735 | 0.889 |
...
*Table 5.7: Own-species F1 recovery under single-species subtractive ablation (seed 42, f1 checkpoint). Recovery = F1(ablated) − F1(full variant); threshold |Δ| > 0.02 for a directional label.*
