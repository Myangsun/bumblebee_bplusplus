...
Section 5.5 establishes *that* synthetic augmentation harms rare species and *that* the LLM filter does not fix the harm. Section 5.6 addresses *why*, combining per-image prediction tracking, embedding-space failure-chain retrieval, judge–classifier disagreement, and a causal ablation of each species' synthetic contribution. The analysis here uses the multi-seed protocol because per-image flip and chain analyses require every seed to evaluate the same images. D5 and D6 per-image flip and chain analyses are [TODO: pending the multi-seed training runs] — Sections 5.6.1–5.6.4 therefore report the analysis on the D3 and D4 variants (unfiltered and LLM-filtered), since those are the variants that demonstrate the LLM-miscalibration claim and have complete training outputs.
...
The operational purpose of the classifier studied in this thesis is to produce trustworthy species-level occurrence records for automated urban biodiversity monitoring — specifically, as the species-level decision stage of the Sensing Garden / Flik pipeline introduced in Section 1.3. Rare-species F1 is the metric that translates most directly into the reliability of those records: each misclassification is either a false positive that inflates a rare-species count or, more consequentially, a false negative that understates it. The cost is asymmetric. An erroneous "B. ashtoni present" record can be corrected by a follow-up ecological survey; an erroneous "B. ashtoni absent" record propagates into multi-site biodiversity indicators and into the conservation-policy and urban-planning decisions those indicators inform, silently under-reporting a species whose true status is exactly what an automated monitoring system is meant to determine.
...
**Single-annotator calibration.** The 150 labels come from one entomologist. The reported LOOCV AUC (0.792) and LLM-expert agreement (κ = 0.132) are conditional on this annotator's judgment; inter-rater reliability has not been measured. Replicating the protocol with ≥ 3 annotators would quantify annotator variance as a fraction of the κ and AUC gaps, and is the first item of Section 7.2.
...
Two improvements address the statistical fragility of the current D6 probe. First, replicate the 150-label annotation protocol with at least three independent annotators drawn from distinct institutional backgrounds — museum curators, field entomologists, taxonomic researchers — report inter-annotator κ and per-feature 95 % CIs, and retrain the probe on the consensus strict label. This quantifies how much of the κ = 0.132 LLM–expert gap and the 0.792 LOOCV AUC reflect annotator subjectivity versus genuine calibration signal (Section 6.7.3). Second, scale the annotation study to 300–500 images with stratified sampling across caste, viewing angle, and LLM tier, tightening the per-species F1-max thresholds currently pinned to as few as 6 expert-strict positives per species (Section 5.4.3). Together these deliver a more robust and better-calibrated probe suitable for deployment.
