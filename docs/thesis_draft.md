...
*Figure 5.6: LLM-strict × expert-strict confusion on the 150-image sample, overall and per species. LLM precision against expert strict = 0.40; recall = 0.62; overall agreement 56 % (84/150), ~7 pp above the 49 % rate expected under independence.*
...
Agreement between the three filter rankings is low. The filter-score scatter of Figure 5.14 plots centroid cosine against probe pass-probability per synthetic, coloured by LLM-strict pass and marked by expert-strict label where available. The probe and centroid scores correlate weakly within species (Spearman ρ ≈ 0.2–0.4 per species) — the probe and the centroid are not simply selecting "near-real" images under different labels; the probe is picking up expert-relevant features that cosine distance alone does not capture. Score-distribution violins split by LLM tier (in `score_violins_by_expert_tier.png`) confirm that the probe distinguishes `strict_pass` from `hard_fail` cleanly on B. sandersoni but not on B. flavidus, again consistent with the expert labelling asymmetry.
