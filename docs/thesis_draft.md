...
*Figure 5.24: Representative D3 failure chains. Shown here: a harmed B. flavidus test image (top-left) and its five nearest D3 training synthetics by BioCLIP cosine similarity. Analogous panels for B. ashtoni and B. sandersoni are in Appendix F. (The on-disk artefact directory `chains_d4_harmed/` uses a legacy code label that corresponds to D3 in the present thesis numbering.)*
