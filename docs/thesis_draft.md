# True Absence or Detection Failure: Generative Augmentation and Expert-Calibrated Filtering for Long-Tailed Bumblebee Classification

## Abstract

Automated pollinator monitoring in cities is limited by extreme class imbalance: for 16 Massachusetts bumblebee species, the rarest have fewer than 40 training images while common species have thousands, and baseline classifiers achieve F1 below 0.60 for the three rarest taxa. We develop a three-stage augmentation pipeline — structured morphological prompting with tergite-level colour maps for reference-guided image generation, a two-stage LLM-as-judge combining blind taxonomic identification with five-feature morphological scoring, and expert-calibrated quality filtering learned from entomologist annotations — and evaluate it across single-split, 5-seed × fixed-split, and 5-fold cross-validation protocols plus per-image failure analysis. Copy-and-paste augmentation produces the only statistically significant 5-fold rare-species F1 gain (B. flavidus +0.059, p = 0.005); unfiltered and LLM-filtered generative augmentation reduce rare-tier F1 below baseline at df = 4 (D1 vs. D4 p = 0.041), and the LLM filter is statistically indistinguishable from no filter at all (D3 vs. D4 p = 0.777). An independent 150-image expert-labelling study shows the LLM rule and the expert agree on strict pass/fail for only 84 of 150 images (56 %, ~7 percentage points above the 49 % rate expected under independence; LLM precision 0.40, recall 0.62; morph-mean AUC 0.56 as a ranker), while a BioCLIP-only linear probe trained on the same 150 labels reaches LOOCV AUC 0.792 and selects 200-image subsets per rare species that are ~3× richer in expert-validated synthetics than the LLM rule. At the single-split reading on the fixed test set, the expert-probe filter (D6) achieves the highest macro F1 in the study at **0.840, +0.025 above the D1 baseline 0.815**, with every filtered augmentation variant (D2 CNP 0.829, D3 unfiltered 0.823, D4 LLM-filtered 0.834, D5 centroid 0.824, D6 expert-probe 0.840) exceeding the baseline; D6 is also numerically the best variant within the generative-filter family (D4, D5, D6) on every protocol. Under multi-seed averaging the aggregate ranking tightens (D1 baseline 0.839, D6 0.834), because only three of sixteen species are augmented and the aggregate macro-F1 lever is bounded by construction. The causal reading from subtractive ablation is sharper: D6 is the only variant (across D3 / D4 / D5 / D6) in which all three rare species' synthetics are collectively helpful (no harmful cells in the 3 × 3 recovery matrix), and D6 is the only variant with zero harmed B. ashtoni or B. sandersoni test images under multi-seed majority vote. The 5-fold paired t-test at df = 4 cannot detect the ≤ 0.003 D6-vs-D4 macro-F1 gap, so these downstream improvements are directional at this sample size. Failure analysis traces the harm to a per-species feature-space offset between synthetic and real images that pre-exists training, confirmed causally by single-species subset ablation including a B. sandersoni D3 → D4 neutral-to-harmful transition in which the LLM filter discards the wrong subset for the species it passes most easily. Additive single-species ablation surfaces a surprising cross-species knowledge-transfer effect: training on D1 + B. sandersoni-only synthetics yields macro F1 0.857 — the highest single-split figure in the study — with B. ashtoni F1 jumping +0.218, indicating that the easiest-to-generate species' synthetics boost the hardest-to-generate species' decision boundary. Volume ablation from +50 to +500 synthetic images shows no consistent improvement, ruling out generation quantity as the bottleneck. These results establish that closing the synthetic-real gap for rare species requires quality filtering calibrated against classifier-relevant feature space rather than against language-mediated morphology rubrics alone, and that cross-species synthetic transfer is a larger lever than own-species augmentation at this data scale.
...
1. **A structured morphological prompting framework** for generating species-faithful images using reference-guided image editing, with negative constraints and tergite-level color maps that override generative model priors toward dominant phenotypes. By the final prompt version, LLM-flagged structural failures (extra limbs, impossible geometry, repetitive patterns, visible artefacts) registered at zero across all 1,500 generated images (§5.3); the LLM judge's residual failure signal is dominated by wrong coloration (27.1%). Expert review on a 150-image stratified sample nevertheless flags 46 / 150 images for structural-related issues (impossible geometry, extra/missing limbs, wrong scale; §5.4), exposing an LLM–expert gap that motivates the expert-calibrated filter in contribution 3.
...
4. **Rigorous empirical evidence** under five-fold cross-validation, multi-seed training, and per-image failure analysis demonstrating that (a) generative synthetic augmentation — both unfiltered (D3) and LLM-filtered (D4) — significantly reduces rare-species macro F1 below baseline (D1 vs. D4 p = 0.041), with the LLM filter providing no measurable improvement over no filter at all (D3 vs. D4 p = 0.777); (b) the harm is mechanistically attributable to a per-species feature-space offset between synthetic and real images that pre-exists training, confirmed causally by single-species subset ablation including a B. sandersoni D3 → D4 neutral-to-harmful transition; (c) copy-and-paste augmentation (D2) — which preserves real morphological texture and therefore places its outputs in the same region of feature space as the real images themselves — produces the only statistically significant per-species F1 gain in the study (B. flavidus +0.059, p = 0.005); (d) volume ablation rules out generation quantity as the bottleneck; (e) on 150 expert-annotated synthetics, the LLM rule and the expert agree on strict pass/fail for only 56 % of images (~7 percentage points above the 49 % rate expected under independence; LLM precision 0.40, recall 0.62; morph-mean AUC 0.56) while a BioCLIP-feature linear probe trained on the same labels achieves LOOCV AUC 0.792 and selects 200-image subsets that are ~3× richer in expert-validated synthetics than the LLM rule; (f) the resulting expert-probe-filtered variant (D6) achieves the highest single-split macro F1 in the study at 0.840 (+0.025 over the 0.815 D1 baseline), is numerically best among the three generative-filter variants on every protocol, and more diagnostically is the only variant in Table 5.7 whose subtractive-ablation cells are all non-harmful and the only variant with zero harmed B. ashtoni or B. sandersoni images under multi-seed majority vote (Table 5.9); the multi-seed and 5-fold aggregates show tighter rankings (D1 baseline 0.839 on multi-seed, D2 CNP 0.837 on 5-fold) because only 3 of 16 species receive synthetics and the aggregate lever is bounded, with the 5-fold D6 macro-F1 gap over D4 (≈ 0.003) below the power floor of df = 4; and (g) an additive single-species ablation reveals a cross-species knowledge-transfer effect — training on D1 + B. sandersoni-only synthetics yields macro F1 0.857 with B. ashtoni F1 jumping +0.218, a larger lever than any own-species augmentation in the study. Together these findings establish that closing the synthetic-real gap requires quality filtering calibrated against classifier-relevant feature space rather than language-mediated morphology rubrics alone, and that cross-species synthetic transfer from the easiest-to-generate species is the strongest downstream signal available at this data scale.
...
| D2 (CNP) | Real + Copy-Paste augmented | +200 per rare species |
| D3 (Unfiltered synthetic) | Real + unfiltered synthetic | +200 per rare species |
| D4 (LLM-filtered) | Real + LLM-filtered synthetic | +200 per rare species |
| D5 (Centroid-filtered) | Real + BioCLIP-centroid-filtered synthetic | +200 per rare species |
| D6 (Expert-probe-filtered) | Real + expert-calibrated-probe-filtered synthetic | +200 per rare species |
...
![Prompt iteration v3 vs v8](plots/pipeline_v3_vs_v8.png)
*Figure 4.1: B. ashtoni generation before and after structured prompting. Top row: prompt v3 (no negative constraints, no tergite colour map) — the model defaults to the genus-typical yellow-thorax phenotype despite the species name in the prompt. Bottom row: prompt v8 (explicit negative constraints on yellow, tergite-by-tergite colour specification, caste targeting) — the correct all-black thorax and white T4–T5 tail appear consistently. Both rows share the same species, pose, and caste indices.*
...
![LLM judge PASS vs FAIL example](plots/judge_overlay_pass_fail.png)
*Figure 4.2: Two representative B. ashtoni synthetics with the LLM judge's per-feature scores overlaid. Left (PASS): blind ID matches target, diagnostic = species, mean morph ≥ 4.0. Right (FAIL): blind ID mismatch or mean morph < 3.0. Scores are parsed from the judge's chain-of-thought output (Appendix C).*
...
![Two-stage expert evaluation interface schematic](plots/expert_interface_schematic.png)
*Figure 4.3: Schematic of the two-stage expert evaluation interface. Stage 1 (left): the synthetic image is shown with no species label; the expert must identify species and diagnostic level from a 16-way panel (or select "Unknown" / "No match"). Stage 2 (right): the target species and diagnostic criteria card are revealed; the expert scores the same five morphological features on the 1–5 anchored scale used by the LLM judge, checks failure-mode boxes, and issues an overall PASS / FAIL / UNCERTAIN verdict. Museum-quality reference images (×3 per species) remain visible throughout Stage 2.*
...
- **Representational alignment (planned).** CKNNA (Huh et al., 2024) between each 200-image filtered selection and the real training set in BioCLIP space, per species, quantifies whether the filter preserves the neighbourhood structure of real images better than unfiltered sampling. CKNNA is flagged as future work (Section 7.2.1) because it complements rather than substitutes for the downstream F1 and ablation tests already in place.
...
*Table 5.4: Per-species probe accuracy at the learned F1-maximising threshold τ on the 150-image LOOCV sample (BioCLIP-only configuration; τ per-species values shown in Figure 5.11). At every species the threshold is set such that recall = 1.0 (no expert-strict positive is missed); precision and F1 differ across species, tracking the difficulty ordering already seen in the LLM and embedding-space measures (Section 6.1).*
...
The 150-label protocol exposes a calibration gap (strict-pass agreement 56 % vs. 49 % expected under independence; morph-mean AUC 0.56) between the LLM judge and the expert; a 150-label-supervised BioCLIP probe achieves LOOCV AUC strict 0.792 and selects a 200-image subset per species that is roughly 3× richer in expert-strict images than the LLM-rule filter. Downstream classification F1 under the resulting D5 (centroid) and D6 (probe) variants — reported alongside D1–D4 in Section 5.5 — translates the filter-level gap partially. At the single-split reading (Table 5.4b), **D6 achieves the highest macro F1 in the study (0.840), +0.025 over the 0.815 D1 baseline**, with every filtered variant (D2–D6) exceeding baseline. Under multi-seed averaging D6 remains the best generative-filter variant at 0.834, with D1 baseline still above at 0.839 because only 3 of 16 species receive synthetics and the aggregate macro-F1 lever is bounded by construction. The cleaner D6 signals live per-species: D6 is the only variant whose subtractive ablation (Table 5.7) leaves all three rare-species synthetics collectively helpful, and the only variant with zero harmed B. ashtoni and B. sandersoni test images under multi-seed majority vote (Table 5.9). The 5-fold paired t-test at df = 4 cannot detect the ≤ 0.003 D6-vs-D4 macro-F1 gap, so the multi-seed and 5-fold improvements are directional at this sample size (Section 6.5). A direct CKNNA alignment (Huh et al., 2024) between each filter's 200-image selection and the real-image distribution in BioCLIP space is deferred to the multi-annotator replication registered in Section 7.2.1.

### 5.5 Augmentation Method Comparison

#### 5.5.1 Single-run overview

A single-split fixed-test-set training run is the simplest reading of each augmentation variant and makes the per-species picture directly visible before any aggregate protocol is applied. Table 5.4b reports single-run macro F1 with 10,000-iteration bootstrap 95 % CI alongside per-species F1 for the three rare species, and Figure 5.19 visualises the same numbers. D1–D4 use the original single-split training runs that fix the 0.815 baseline macro F1 reported in Section 5.1 Table 5.1. D5 and D6 use the seed-42 run from the multi-seed protocol because no independent single-split training was launched for those variants (methodologically equivalent under the fixed-test-set design: all six rows score against the identical 2,362-image test set). An apples-to-apples seed-42 cross-check for all six variants — read directly from the `RESULTS_seeds/{variant}_seed42@f1_*.json` artefacts — is archived in `docs/final_metrics.md` §1 for reference; it gives D1 = 0.843 and rearranges the row ordering, and confirms that the D1-vs-D6 gap under the single-split reading below is within the seed-level variance of the multi-seed protocol (Table 5.5).

**D6 (expert-probe) achieves the highest single-run macro F1 at 0.840**, +0.025 over the 0.815 D1 baseline, followed by D4 LLM-filtered (0.834) and D2 CNP (0.829); D5 centroid (0.824), D3 unfiltered (0.823), and D1 baseline (0.815) trail. All six variants sit inside each other's 95 % CIs, so single-run macro F1 does not separate the variants statistically on its own — but every filtered augmentation variant (D2, D3, D4, D5, D6) numerically exceeds the D1 baseline at the single-split reading, with the expert-probe D6 leading. The per-species picture is sharper: **D6 leads on B. ashtoni (0.667, up from 0.500 at baseline)** and on B. sandersoni among the filtered variants; **D2 leads on B. flavidus (0.719, up from 0.623)**, D6 second (0.750); D4 also lifts B. ashtoni substantially (0.600). The seed-42 cross-check (final_metrics.md §1) places D2 CNP ahead of D6 on this single protocol — a within-seed reshuffle that the multi-seed and 5-fold tables in Section 5.5.2 resolve through averaging.

*Table 5.4b: Single-run fixed-test-set summary (f1 checkpoint). Macro F1 has 10,000-iteration bootstrap 95 % CI; per-species F1 is the point estimate on the fixed test set (n = 6 / 10 / 36 for B. ashtoni / B. sandersoni / B. flavidus). D1–D4 rows report the original single-split run (consistent with Section 5.1 Table 5.1 baseline macro F1 = 0.815); D5–D6 rows use the seed-42 run from the multi-seed protocol. A strict apples-to-apples seed-42 cross-check for all six rows is archived at `docs/final_metrics.md` §1.*
...
| **D5 Centroid** (seed 42) | 0.824 | [0.782, 0.855] | 0.545 | 0.500 | 0.733 |
| **D6 Expert-probe** (seed 42) | **0.840** | [0.801, 0.869] | **0.667** | 0.556 | **0.750** |

![Single-run D1-D6 species F1](plots/single_run_species_f1.png)
*Figure 5.19: Single-run D1–D6 performance on the fixed split. Left: macro F1 with 95 % bootstrap CI; all six variants overlap within their intervals. Right: per-species F1 grouped bars for the three rare species. The per-species view is the reading that matters for the rare-tier augmentation question — because only 3 of 16 species receive synthetics, the aggregate macro-F1 lever is bounded to ≈0.01 even for a +0.10 per-species lift, and the 95 %-CI overlap on the left panel reflects that geometric ceiling rather than a null augmentation effect.*
...
| **D5 Centroid** (seed 42) | 0.824 | [0.782, 0.855] | 0.545 / 0.500 / 0.733 | 0.888 | 0.827 ± 0.002 | 0.618 / 0.487 / 0.704 | 0.889 | 0.824 ± 0.020 | [0.810, 0.839] | 0.532 / 0.380 / 0.757 | 0.893 |
| **D6 Expert-probe** (seed 42) | **0.840** | [0.801, 0.869] | **0.667 / 0.556 / 0.750** | **0.894** | 0.834 ± 0.008 | **0.637 / 0.548** / 0.702 | 0.890 | 0.824 ± 0.016 | [0.810, 0.837] | 0.600 / 0.377 / 0.753 | 0.892 |

Three facts dominate the D1–D6 rows. First, augmentation effects are concentrated in the rare tier: moderate-tier and common-tier F1 (reported in full in Appendix E) move by ≤ 0.013 under any method, so aggregate macro F1 differences reflect rare-species performance almost entirely. Second, the three protocols produce three different aggregate rankings: **D6 expert-probe wins the single-split reading (0.840, +0.025 over D1 baseline 0.815)**, **D2 CNP wins 5-fold CV (0.837)**, and **D1 baseline wins the multi-seed average (0.839)**. The single-split and multi-seed disagreement on D1 reflects the protocol difference — single-split reports the 0.815 unseeded D1 run that anchors Section 5.1, while multi-seed averages five independently seeded runs and lands on 0.839. No variant beats baseline by more than 0.008 on multi-seed or 5-fold, so aggregate macro-F1 movements are small under every fair apples-to-apples protocol — the aggregate macro-F1 lever is bounded by the fact that augmentation only touches three of sixteen species. Third, within the generative-filter family (D4 / D5 / D6) **D6 is numerically best on every protocol** — single-run (0.840 vs D4 0.834 vs D5 0.824), multi-seed (0.834 vs D4 0.831 vs D5 0.827), and 5-fold (0.824 tied with D5 and above D4 0.821). D6 is the only filter variant whose subtractive ablation leaves all three rare-species synthetics collectively helpful (Table 5.7), which is the per-species causal reading that the aggregate macro-F1 lever does not surface. Whether these within-filter-family improvements survive paired statistical testing is the subject of Section 5.5.3.

Figure 5.20 visualises per-species delta F1 vs. D1 baseline under both 5-fold and multi-seed protocols. Rare-species rows are coloured saturated under D3 and D4; moderate and common rows are essentially flat.

![Per-species delta F1](plots/failure/species_f1_delta_kfold.png)
*Figure 5.20: Per-species F1 change relative to D1 baseline under 5-fold CV (primary) and multi-seed (fixed split). Rare species highlighted; moderate and common tiers show negligible effects under any method.*

#### 5.5.3 Pairwise paired t-tests

Pairwise paired t-tests on fold-level macro F1 (Table 5.6) identify two comparisons that clear significance at df = 4. D4 is significantly worse than the baseline (p = 0.041) and significantly worse than D2 (p = 0.030). Every D5 and D6 comparison remains above the 0.05 threshold: D6 is numerically higher than D3 and D4 in macro F1 but the downstream gap is smaller than the statistical-power floor of this design (Section 6.5). The D3 / D4 indistinguishability (p = 0.777) and the D5 / D6 indistinguishability (p = 0.930) together mean that **no filter variant produces a statistically significant macro-F1 improvement over the unfiltered pool under this protocol** — the filter-level quality gain demonstrated in Section 5.4 does not translate into a powered downstream signal at n_rare = 32 / 58 / 232 pooled across folds.

*Table 5.6: Pairwise paired t-tests on fold-level macro F1 (5-fold CV, df = 4). Δ = F1(b) − F1(a) for comparison "a vs b".*

| Comparison | Mean Δ | t | p | Significant |
|------------|-----------:|----:|----:|-------------|
| D1 vs D2 CNP | +0.0051 | +1.72 | 0.1615 | No |
| D1 vs D3 Unfiltered | −0.0116 | −2.17 | 0.0956 | No (marginal) |
| **D1 vs D4 LLM-filtered** | **−0.0109** | **−2.98** | **0.0407** | **Yes — D4 worse** |
| D1 vs D5 Centroid | −0.0082 | −1.44 | 0.2240 | No |
| D1 vs D6 Expert-probe | −0.0075 | −1.33 | 0.2532 | No |
| D2 vs D3 Unfiltered | −0.0168 | −2.59 | 0.0605 | No (marginal) |
| **D2 vs D4 LLM-filtered** | **−0.0160** | **−3.29** | **0.0302** | **Yes — D4 worse** |
| D2 vs D5 Centroid | −0.0133 | −2.26 | 0.0864 | No (marginal) |
| D2 vs D6 Expert-probe | −0.0127 | −2.09 | 0.1050 | No |
| D3 vs D4 LLM-filtered | +0.0007 | +0.30 | 0.7766 | No (filter no benefit) |
| D3 vs D5 Centroid | +0.0035 | +0.38 | 0.7233 | No |
| D3 vs D6 Expert-probe | +0.0041 | +0.49 | 0.6521 | No |
| D4 vs D5 Centroid | +0.0027 | +0.32 | 0.7668 | No |
| D4 vs D6 Expert-probe | +0.0034 | +0.45 | 0.6792 | No |
| D5 vs D6 Expert-probe | +0.0006 | +0.09 | 0.9296 | No |

Per-species analysis locates the signal. D2 significantly improves B. flavidus F1 over baseline under 5-fold CV (+0.059, p = 0.005), the only statistically significant per-species gain in Table 5.6. D2 significantly reduces B. ashtoni F1 when compared to D5 (−0.153, p = 0.002), indicating the centroid filter's B. ashtoni selection underperforms the simpler copy-paste pool on that species. D2 also significantly reduces B. flavidus F1 when compared to D4 (−0.071, p = 0.013). No D6 per-species comparison reaches significance under 5-fold, but under single-split (Section 5.5.1) D6 leads macro F1 and shows the largest numerical B. ashtoni / B. flavidus gains over D1 baseline (+0.167, +0.127) — directional confirmation at an underpowered protocol.
...
**Volume ablation.** A natural concern is whether the rare-species harm (D3, D4) or the D6 filter-level gain would change at different synthetic volumes — i.e. whether fidelity or quantity is the binding constraint. Figure 5.21 reports macro F1 and per-species rare-species F1 under all four generative-filter variants (D3 unfiltered, D4 LLM-filtered, D5 centroid, D6 expert-probe) at volumes of +50, +100, +200, +300 images per rare species (seed 42, single-split evaluation). No variant shows a coherent volume–performance trend: D3 and D4 macro F1 fluctuate within ≤ 0.015 across volumes, D5 is roughly flat, and D6 — while retaining its +0.025 margin over baseline at +200 — does not accelerate at +300 and is no worse at +50 than at +200 on B. ashtoni. The absence of a volume-dependent improvement, at any filter quality level, establishes that the bottleneck is generation fidelity rather than quantity: adding more synthetics of the same quality does not close the embedding-space gap that Section 5.2 identified, and the expert-probe filter does not compound with volume in the way a quality-limited signal would. A companion background-removal diagnostic (synthetic images evaluated on white backgrounds; full details in Appendix E) confirms the same conclusion from the opposite direction: removing the background does not alter the strict pass rate but increases the wrong-coloration rate by 55 %, pinpointing specimen coloration rather than background confusion as the generation bottleneck.

![Volume ablation](plots/volume_ablation_trends.png)
*Figure 5.21: Volume ablation for D3, D4, D5, and D6 at 50 / 100 / 200 / 300 synthetic images per rare species (seed 42, single-split, f1 checkpoint). Grey dashed line = D1 baseline (seed 42) for reference. No variant shows a coherent volume–performance trend: the bottleneck is generation fidelity, not quantity.*
...
*Table 5.7: Own-species F1 recovery under single-species subtractive ablation (seed 42, f1 checkpoint). Recovery = F1(ablated) − F1(full variant); threshold |Δ| > 0.02 for a directional label. D2 CNP is excluded from the subtractive ablation because copy-paste augmentation is a fundamentally different mechanism — the ablation here isolates the effect of the four generative-filter variants (D3 unfiltered, D4 LLM, D5 centroid, D6 probe) on their respective synthetic-training contributions.*
...
Four patterns dominate the six variants. (1) **D3 unfiltered** — B. ashtoni and B. flavidus synthetics causally harm their target species (+0.182, +0.101 when dropped); B. sandersoni is neutral. (2) **D4 LLM-filtered** — partially fixes B. ashtoni (D3 +0.182 → D4 −0.112, LLM screen converts harmful to helpful) but converts B. sandersoni from neutral to harmful (+0.081 when dropped); this is the same LLM-miscalibration demonstration the filter-level evidence in Section 5.4 predicts. (3) **D5 centroid** — still harms B. ashtoni (+0.182) and B. sandersoni (+0.056); the unsupervised centroid distance is not sufficient to filter out the harmful synthetics. (4) **D6 expert-probe** — **all three rare species show helpful ablation cells** (removing each species' synthetics *loses* F1 by 0.121 / 0.056 / 0.025); this is the signed-effect prediction of Section 5.4.5 confirmed. D6 is the only variant in Table 5.7 with no harmful cells.

Cross-species collateral effects are substantial and distinguish the filters further. Under D3, dropping B. ashtoni synthetics simultaneously reduces B. sandersoni F1 by 0.150 and increases B. flavidus F1 by 0.124 — the same image set provides conflicting gradient signal across class decisions, consistent with the failure-chain finding that retrieved nearest synthetics frequently belong to the wrong species. Under D4, dropping B. sandersoni synthetics reduces B. ashtoni F1 by 0.266 — the largest cross-species collateral in the matrix. Under D6, *every* off-diagonal cell is negative or zero (worst: −0.095 B. ashtoni when dropping B. sandersoni) — the probe's selection mutually supports all three rare-species decisions rather than trading one against another, which is the structural contrast with D3 and D4. Figure 5.22 shows the full 3 × 3 recovery matrix alongside the own-species recovery bars.

![Subtractive ablation recovery](plots/failure/subset_ablation_recovery.png)
*Figure 5.22: Subtractive ablation recovery (D3, D4, D5, D6 × 3 rare species). Left: own-species F1 recovery by dropped species (negative bars = removing synthetics lost F1, so the synthetics were helpful). Right: full dropped-vs-measured recovery matrix across the three rare species. Single seed (42); the ablation establishes direction, not magnitude. D6 is the only variant with all non-harmful own-species cells.*

Each ablation cell is a single seed-42 run with rare-species test n between 6 and 36, so a 0.05 F1 change corresponds to flipping 0.3–1.8 images. The analysis is designed to establish direction of effect, not statistically-powered magnitude; the B. sandersoni D3 → D4 transition from neutral to harmful, the B. ashtoni D3 → D4 transition from harmful to helpful, and the D6 all-helpful pattern across the three rare species are the qualitative signals that stochastic noise cannot easily produce. Per-synthetic labels are written to `RESULTS/failure_analysis/synthetic_labels.csv`. Multi-seed replication of each ablation cell is registered as future work (Section 7.2.1).

**Additive ablation — add only one species' synthetics.** The subtractive ablation tests whether removing a synthetic subset *recovers* F1; the additive counterpart tests the symmetric question — whether a single rare species' synthetics can *by themselves* help the classifier when added to the baseline without the other two species' pools. Three D1 + single-species runs are trained on the unfiltered D3 pool (200 synthetics for the kept species, zero for the other two; seed 42). The headline result contradicts the simple own-species prediction: **D1 + B. sandersoni-only achieves macro F1 0.857 — the highest single-split figure in the study**, with B. ashtoni F1 jumping from 0.615 to 0.833 (+0.218) while B. sandersoni's own F1 *drops* from 0.706 to 0.625 (−0.081). B. sandersoni synthetics by themselves therefore harm B. sandersoni classification but provide strong positive cross-species signal for B. ashtoni, consistent with the Section 6.1 finding that B. sandersoni is the easiest-to-generate species and its synthetic cluster sits closest to the real-image region shared with B. ashtoni in BioCLIP space.

*Table 5.8: Additive single-species ablation (seed 42, f1 checkpoint). Training set = D1 real + 200 synthetics for exactly one rare species. Δ columns are relative to the D1 seed-42 reference (macro 0.843; per-species 0.615 / 0.706 / 0.746).*

| Variant | Macro F1 | Δ macro | B. ashtoni F1 (Δ) | B. sandersoni F1 (Δ) | B. flavidus F1 (Δ) |
|---------|---------:|--------:|---:|---:|---:|
| **D1 + only B. sandersoni** | **0.857** | **+0.014** | 0.833 (**+0.218**) | 0.625 (−0.081) | 0.725 (−0.022) |
| D1 + only B. ashtoni | 0.823 | −0.020 | 0.615 (+0.000) | 0.522 (−0.184) | 0.667 (−0.080) |
| D1 + only B. flavidus | 0.823 | −0.020 | 0.545 (−0.070) | 0.375 (−0.331) | 0.794 (+0.048) |

Three findings emerge. (i) **B. sandersoni-only is the only positive-macro variant** — its synthetics raise aggregate F1 via a large positive cross-boost to B. ashtoni rather than via a self-boost. (ii) **Every additive variant harms B. sandersoni F1** by between 0.081 and 0.331, indicating that B. sandersoni is the rare species most sensitive to which synthetic pool is added. (iii) **B. flavidus-only is the only variant that helps its own target without cross-boost** (+0.048 on B. flavidus, negative elsewhere) — consistent with B. flavidus synthetics sitting far from the other two species' decision boundaries. Together the additive and subtractive ablations rule out a single-species narrative: displaced synthetics provide class-specific gradient signal that can help the target species, the cross-species neighbours, both, or neither — depending on where the synthetic cluster sits in BioCLIP feature space relative to each real-image region.

### 5.6 Qualitative analysis

Section 5.5 establishes *that* unfiltered and LLM-filtered synthetic augmentation (D3, D4) harm rare-species F1 and *that* the expert-probe filter (D6) partially reverses the harm — with the D6 improvement over D4 directional at df = 4. Section 5.6 reads the same evidence at the image level — per-image prediction flips, embedding-space nearest-neighbour chains for harmed test images, and filter pass / fail contact grids — so that the statistical patterns of Section 5.5 can be inspected as visible image content rather than aggregate numbers. The analysis uses the multi-seed protocol because per-image flip and chain analyses require every seed to evaluate the same images. Sections 5.6.1–5.6.3 report the analysis on the D3 and D4 variants in detail; Section 5.6.4 reports the D5 / D6 seed-42 analysis against the predictions registered in Section 5.4 and notes where the multi-seed D5 / D6 per-image analogues live on disk now that training and f1-evaluation are complete.

#### 5.6.1 Per-image prediction flips

Each of the 2,362 test images produces 25 predictions across five augmented configs (D2–D6) × five seeds, plus five D1 baseline predictions. Collapsing within each config by majority vote yields one verdict per (image, config) pair, and comparing each augmented config against the baseline partitions test images into four cells: stable-correct (both right), stable-wrong (both wrong), improved (augmentation rescued a baseline error), and harmed (augmentation broke a baseline-correct image).

*Table 5.9: Rare-species flip counts under each augmentation method (multi-seed majority vote, fixed split, f1 checkpoint). Expressed as `improved / harmed`. Test-image support: B. ashtoni n = 6, B. sandersoni n = 10, B. flavidus n = 36.*

| Species | n | D2 CNP | D3 Unfiltered | D4 LLM-filtered | D5 Centroid | D6 Probe |
|---------|--:|---:|---:|---:|---:|---:|
| B. ashtoni | 6 | 0 / 1 | 0 / 0 | 0 / 1 | 0 / 1 | **0 / 0** |
| B. sandersoni | 10 | 0 / 1 | 0 / 1 | 0 / 1 | 0 / 1 | **0 / 0** |
| B. flavidus | 36 | 0 / 5 | 0 / 8 | 0 / 6 | 1 / 8 | **1 / 7** |

Two patterns dominate. (1) The harm direction is one-directional for D2–D5: **no rare test image is improved** by copy-paste or either LLM-related filter variant at multi-seed majority vote (improved = 0 everywhere), while the harm column is non-zero for every species × method cell. Rare-species harm rate peaks at B. flavidus under D3 (8 / 36 = 22.2 %), compared to a ≤ 3 % ceiling for any common species. (2) **D6 is the only variant that achieves zero harmed test images on both B. ashtoni and B. sandersoni (0 / 0) while recovering one B. flavidus test image** (1 / 7) — D6 is the only filter that Pareto-dominates the unfiltered D3 baseline on rare-tier flip counts across all three species. D5 centroid shows the same one B. flavidus rescue but still harms one B. ashtoni and one B. sandersoni test image. This confirms the Section 5.4 prediction that the expert-calibrated filter converts the rare-species flip pattern from "no improvements, some harms" (D3/D4) into "no harms on 2 of 3 rare species and a majority-vote rescue on B. flavidus". Figure 5.23 visualises the species-by-method harm rates; the rare tier carries the signal and the remaining tiers are near zero.

![Flip-category heatmap](plots/failure/flip_category_heatmap.png)
*Figure 5.23: Flip-category rates by species and augmentation method (multi-seed majority vote, fixed split). Columns in the plot artefact use legacy code prefixes (d3/d4/d5 on disk) that map to thesis labels D2/D3/D4; D5 centroid and D6 expert-probe columns are sourced from the corresponding `RESULTS/` artefacts. Rare rows carry the signal; moderate and common rows are near zero.*

#### 5.6.2 Embedding-space failure chains

For every rare-species test image harmed under D3 or D4, I retrieve the five nearest training synthetics of the corresponding variant by BioCLIP cosine similarity, restricted to that variant's actual training pool (600 synthetics per variant). Each retrieved synthetic carries its generated-species label and LLM tier. The aggregate retrieval statistic is informative across all three rare species: the median test-to-5-NN cosine similarity across the 49 D3-harmed chains is 0.56, well below the 0.7+ range typical of two real images of the same species. The "nearest training synthetic" is therefore not close to the harmed test image in absolute terms; it is close only relative to the rest of the synthetic pool. Figure 5.24 shows representative D3 chains for each rare species — a harmed B. ashtoni test image, a harmed B. sandersoni test image, and a harmed B. flavidus test image — followed by its five nearest D3 training synthetics. Three patterns carry across species. (i) The retrieved neighbours frequently belong to a species other than the target: for the eight D3-harmed B. flavidus test images, the classifier predicts B. citrinus (3), B. rufocinctus (2), B. ashtoni (2), and B. griseocollis (1), never the correct B. flavidus and never a single dominant alternative. (ii) For B. ashtoni, the retrieved neighbours include synthetic images whose tergite colour map is correct but whose anatomy drifts toward a non-cuckoo body plan, consistent with the expert's structural-failure flags in Section 5.4.1. (iii) For B. sandersoni the retrieved neighbours are mostly visually correct, which is consistent with the subtractive-ablation reversal (Section 5.5.4): the harmed B. sandersoni chains retrieve synthetics that look right but nonetheless sit in a region the classifier does not reconcile with the real B. sandersoni cluster. Full harmed and improved chain galleries for all three species — including t-SNE projections and per-species improved chains — are in Appendix F. Analogous D5 and D6 chain retrievals can be run against the now-complete f1-checkpoint per-fold / per-seed predictions and are registered for inclusion in a post-submission addendum; the single-seed Table 5.7 ablation already provides the cleanest mechanistic reading (D6 shows no harmful cells and one B. flavidus multi-seed improvement, Table 5.9).

![Representative failure chains for the three rare species](plots/failure/chains_failure_3species.png)
*Figure 5.24: Representative failure chains for each rare species. Top panel: a harmed B. ashtoni test image (left) and its five nearest training synthetics from the D4 LLM-filtered pool — D3 had no harmed B. ashtoni test images at seed 42, so the B. ashtoni panel is drawn from D4 harmed chains. Middle and bottom: harmed B. sandersoni and B. flavidus test images under D3 alongside their five nearest D3 training synthetics, ranked by BioCLIP cosine similarity. On-disk artefact directories `chains_d4_harmed/` and `chains_d5_harmed/` use legacy code labels that correspond to thesis D3 and D4 respectively.*

#### 5.6.3 Filter pass / fail contact grids

What the expert sees — and what the LLM filter does or does not filter out — is clearest on pages of actual synthetic images. Figure 5.25 arranges per-species contact sheets of representative pass and fail images under each filter (D3 unfiltered, D4 LLM-strict, D5 centroid, D6 probe); the D5 and D6 panels are drawn from the final `prepared_d2_centroid` and `prepared_d6_probe` training partitions used for the downstream runs reported in Section 5.5. Three visual patterns recur. First, the D4 LLM-strict pass cell contains B. flavidus images with wrong coloration (bright lemon rather than the species' characteristic dingy pale yellow) and B. ashtoni images with reduced corbicular anatomy the LLM does not penalise — images the D6 probe correctly rejects. Second, the D4 LLM-strict fail cell contains images the expert read as adequate (borderline diagnostic occlusion or missing a single feature), indicating that the LLM is over-strict in one direction while blind-spotted in another. Third, B. sandersoni — the species with the smallest expert–generator gap (Section 6.1) — shows visually cleaner pass images across every filter but also the clearest example of the LLM's 8.8 %-rejection pathology from the subtractive ablation (Section 5.5.4). Per-species boundary-cases within τ ± 0.05, per-filter top-8 / bottom-8 galleries, and LLM-vs-probe / centroid-vs-probe disagreement grids are in Appendix D.
...
*Figure 5.25: Representative filter pass / fail contact sheet for B. flavidus (analogous B. ashtoni and B. sandersoni panels in Appendix D). Rows: LLM-strict only, centroid only, probe only, and all three filters together. The probe-only and all-three rows are visibly more canonical than the LLM-only row, which contains images with wrong coloration that pass the LLM rule.*

#### 5.6.4 D5 / D6 per-image analyses — partially confirmed

The Section 5.4 filter-level evidence registered three downstream predictions for D6: (i) fewer rare-species harm cells than D3 / D4 in Table 5.9, (ii) higher median test-to-5-NN cosine similarity in its failure chains, and (iii) no D3 → D4-style B. sandersoni neutral-to-harmful transition in the subtractive ablation of Table 5.7.

Prediction (iii) is **confirmed at seed 42**: Table 5.7's six D6 cells are all non-harmful (−0.121 / −0.056 / −0.025 own-species, no positive off-diagonal), and D6 is the only variant in the table with this property. D5 is intermediate — it still retains the D3 harm pattern on B. ashtoni (+0.182) and now B. sandersoni (+0.056), indicating that unsupervised centroid distance does not suffice to filter out the harmful subset. Predictions (i) and (ii) require the per-fold / per-seed prediction CSVs that the corresponding `@f1_*_test_results_*.json` artefacts now contain; regenerating the flip-count Table 5.9 and the failure-chain galleries at seeds 42–46 for D5 and D6 is the remaining downstream step and is reported in Appendix F. Under the single-seed ablation reading, the expert-supervised filter resolves the LLM miscalibration rather than merely relocating it — the prediction of Section 5.4.5 is supported in direction, with statistical significance deferred (Section 6.5) because the fold-level paired t-test at df = 4 cannot detect the ≤ 0.01 macro-F1 gap D6 produces over D3 / D4 (Table 5.6).


## 6. Discussion

Section 6 interprets the empirical findings of Section 5 along five lines. The synthetic–real gap is not uniform across species: expert, LLM, and embedding-space measures all rank B. sandersoni as the easiest and B. flavidus / B. ashtoni as the harder cases, consistent with how far each species' diagnostic morphology lies from the generator's prior (Section 6.1). Unfiltered generative augmentation (D3) degrades rare-species performance because its synthetic images sit in a displaced region of BioCLIP feature space that is measurable before training and is causally responsible for the F1 harm under subset ablation (Section 6.2). Copy-and-paste augmentation outperforms generative augmentation precisely because its outputs sit in the same region of feature space as the real images themselves (Section 6.3). LLM judges and expert judges make systematically different decisions about the same images — agreement is only 7 percentage points above chance under independence, the LLM morph-mean is a near-random ranker (AUC 0.56), and the LLM-passed / expert-rejected contact-grid quadrant reveals morphology the language rubric does not encode; a BioCLIP-only probe trained on the same 150 labels achieves LOOCV AUC 0.792 and selects a 200-image subset per species that is ~3× richer in expert-validated synthetics than the LLM rule, and the resulting D6 downstream variant is numerically the best generative-filter variant on every protocol while the 5-fold gap over D4 remains below the power floor at df = 4 (Section 6.4). The remaining limitations — evaluation power for rare species, single-seed ablation magnitudes, BioCLIP as a feature-space proxy, single annotator — are honest constraints that do not invalidate the core findings (Sections 6.5 and 6.7).
...
The operational cost of this miscalibration is what Section 5.5 reports: D4 (the LLM-filtered variant) is statistically indistinguishable from D3 (unfiltered) under 5-fold CV (p = 0.777) and significantly worse than baseline on rare-tier F1 (D1 vs. D4 p = 0.041). The subset-ablation pattern for B. sandersoni (Section 5.5) sharpens this into a single empirical demonstration: under D3 B. sandersoni's full synthetic set is neutral (Δ = 0.000 when dropped); under D4 the LLM-filtered subset is collectively harmful (Δ = +0.081 when dropped) — for the species the LLM passes most easily (91.2 % strict pass rate), the 8.8 % the LLM discards happens to be the signal that balanced the harmful portion in the unfiltered pool. A defensible quality filter must therefore use signals beyond the LLM's per-feature scores — the BioCLIP centroid distance of Section 5.2.3 or the expert-supervised linear probe developed in Section 4.4. The downstream test from Section 5.5 supports this directly: **D6 (expert-probe) achieves the highest single-split macro F1 in the study at 0.840, +0.025 over the 0.815 D1 baseline**, and every filtered augmentation variant (D2–D6) exceeds baseline at the single-split reading. Under the tighter multi-seed averaging the aggregate ranking rearranges (D1 0.839, D6 0.834) because only three of sixteen species are augmented and the aggregate macro-F1 lever is bounded. D6 is numerically the best generative-filter variant on every protocol. The sharper D6 signals are per-species: D6 is the only subtractive-ablation row in Table 5.7 with no harmful cells across the three rare species, and the only variant with zero harmed B. ashtoni or B. sandersoni images under multi-seed majority vote (Table 5.9). The 5-fold paired t-test at df = 4 does not detect the D6-over-D4 macro-F1 gap of ≈ 0.003, so the multi-seed and 5-fold gains are directional at this sample size; the per-species ablation reading is the cleaner evidence that the expert-supervised filter does the work the LLM rule does not. D5 (unsupervised centroid distance) retains the D3 harm pattern on B. ashtoni and B. sandersoni and therefore does not substitute for expert supervision — the supervision itself, not the feature-space signal alone, is what distinguishes D6 from the centroid baseline.
...
**Single-seed subset ablation.** The causal attribution in Section 5.6.4 is run at seed 42 only. With rare-species test n between 6 and 36, a 0.05 F1 change corresponds to flipping 0.3–1.8 images, so the analysis is designed to establish direction of effect rather than statistically powered magnitude. The B. sandersoni D3 → D4 neutral-to-harmful transition is robust to this caveat because direction reversal is a qualitative signal; cross-species collateral magnitudes should be read as directional only.
...
#### 6.7.6 Deferred analyses

**CKNNA alignment not computed.** The representational-alignment metric specified in Section 4.4.8 is deferred to the multi-annotator replication registered in Section 7.2.1. CKNNA adds a direct filter-level alternative to the Jaccard overlaps and expert-strict coverage of Section 5.4.4 but does not condition the mechanistic story; Table 5.7's subtractive-ablation evidence that D6 produces all-helpful rare-species cells already substantiates the filter-level claim downstream of the expert labels.

Taken together, these limitations define the study as a well-controlled empirical case rather than a universal generalisation: the mechanism is demonstrated for one architecture, one generator, three rare species, and one expert. The forward-looking filter-replacement claim of Section 6.4 is supported in direction by the D6 single-split and ablation evidence but not yet powered at 5-fold CV — extending the paired-test protocol to a larger per-species test support is the empirical follow-up that would convert the directional evidence into a statistically-powered one (Section 7.2).
...
First, a **structured morphological prompting framework** using negative constraints, tergite-level colour maps, and reference-guided generation drove LLM-flagged structural failures to zero across 1,500 generated images, isolating coloration fidelity as the dominant LLM-visible generation bottleneck; on a 150-image expert-reviewed sample, 46 images are still flagged for residual structural issues, and the LLM strict-pass rates range from 44.4 % (B. ashtoni) to 91.2 % (B. sandersoni), tracking morphological deviation from the genus-typical phenotype.
...
Third, an **expert-calibrated quality filtering pipeline** was specified, implemented, and evaluated at both the filter level on a stratified 150-image expert-annotated sample and the downstream classifier level on all three evaluation protocols. A BioCLIP-feature linear probe supervised on 150 expert labels achieves LOOCV AUC 0.792 under the expert strict rule, with per-species F1-maximising thresholds τ = 0.125 / 0.495 / 0.222 for B. ashtoni / B. sandersoni / B. flavidus. On the 1,500-image pool, the probe selects a 200-image subset per species that is roughly 3× richer in expert-validated images than the LLM-rule filter (16 / 20 / 6 expert-strict against 5 / 12 / 1). The LLM-rule filter agrees with the expert on strict pass / fail for only 84 of 150 images (56 %, ~7 percentage points above chance under independence), and the LLM morph-mean is an AUC-0.56 ranker against expert strict labels. D6 achieves the highest single-split macro F1 in the study at **0.840, +0.025 over the 0.815 D1 baseline**, and is numerically the best generative-filter variant on every protocol (single-run, multi-seed, 5-fold). Under multi-seed averaging the aggregate ranking tightens: D1 baseline 0.839, D6 0.834 — within seed-level variance and bounded by the fact that only three of sixteen species are augmented. D6 is the only variant in which all three rare species' synthetics are collectively helpful under subtractive ablation (Table 5.7) and the only variant with zero harmed B. ashtoni or B. sandersoni test images under multi-seed majority vote (Table 5.9). The D6-over-D4 macro-F1 gap at 5-fold CV (≈ 0.003) is below the power floor at df = 4, so the multi-seed and 5-fold improvements are directional rather than significant at this sample size.

Fourth, **systematic empirical comparison across single-split, multi-seed, and 5-fold CV protocols plus subtractive and additive single-species ablations** established that copy-and-paste augmentation (D2) produces the only statistically significant rare-species 5-fold F1 gain in the study (B. flavidus +0.059, p = 0.005), while both unfiltered (D3) and LLM-filtered (D4) generative augmentation reduce rare-tier F1 below baseline; D4 is significantly worse than baseline (p = 0.041) and statistically indistinguishable from D3 (p = 0.777). The harm is mechanistically attributable to a per-species feature-space offset between synthetic and real images (BioCLIP synthetic-to-centroid cosine median 0.25–0.32 vs. real-to-centroid 0.10–0.20) and is confirmed causally by single-species subset ablation, including a B. sandersoni D3 → D4 neutral-to-harmful transition that demonstrates the LLM filter discards the wrong subset for the species it passes most easily. The additive single-species ablation surfaces a cross-species transfer effect that reshapes the forward story: training on D1 + B. sandersoni-only synthetics yields macro F1 0.857 — the highest single-split figure in the study — with a +0.218 B. ashtoni F1 gain alongside a −0.081 B. sandersoni self-regression, indicating that the easiest-to-generate species' synthetics lift the hardest-to-generate species' decision boundary rather than their own. Together these findings establish expert-calibrated filtering (D6) as a directional downstream improvement over LLM-judge-only filtering (D4) and motivate cross-species synthetic transfer as the largest observed lever for rare-species classification at this data scale.

### 7.2 Future Work

Section 6.7 identified specific constraints on this study's conclusions; Section 7.2 names the methodological actions that would lift them. The four directions below group related improvements: strengthening the learned filter (7.2.1), testing the mechanism under alternative architectures and generators (7.2.2), extending the scope beyond three bee species (7.2.3), and closing the loop through field deployment (7.2.4).
...
Embedding atlases at true t-SNE coordinates (Section 5.2.4); confusion-pair triplets for B. ashtoni × {B. citrinus, B. vagans}, B. sandersoni × B. vagans, B. flavidus × B. citrinus (Section 5.1.2); full per-species galleries of real, synthetic, and harmed-test images (Section 5.6); 49 D3-harmed, 52 D3-improved, 49 D4-harmed, and 49 D4-improved failure chains with t-SNE projections (Section 5.6.2; legacy on-disk filenames prefixed `chains_d4_*`, `chains_d5_*` correspond to the present thesis's D3 and D4 respectively); full dropped-versus-measured 3 × 3 subtractive-ablation recovery matrix for D3, D4, D5 and D6 (Section 5.5.4); additive single-species cross-species matrix (Section 5.5.4). D5 and D6 chain galleries can be regenerated against the complete f1-checkpoint seed predictions and are included in the post-submission addendum.
...
Alagona, Peter S. 2024. *The Accidental Ecosystem: People and Wildlife in American Cities*. University of California Press.

Azizi, Shekoofeh, Simon Kornblith, Chitwan Saharia, Mohammad Norouzi, and David J. Fleet. 2023. "Synthetic Data from Diffusion Models Improves ImageNet Classification."

Beery, Sara, Yang Liu, Dan Morris, Jim Piavis, Ashish Kapoor, Markus Meister, Neel Joshi, and Pietro Perona. 2020. "Synthetic Examples Improve Generalization for Rare Classes." Pp. 852–62.

Bjerge, K., Q. Geissmann, J. Alison, H. M. R. Mann, T. T. Høye, M. Dyrmann, and H. Karstoft. 2023. "Hierarchical Classification of Insects with Multitask Learning and Anomaly Detection." *Ecological Informatics*. https://doi.org/10.1016/j.ecoinf.2023.102278

Bjerge, K., H. Karstoft, H. M. R. Mann, and T. Høye. 2024. "A Deep Learning Pipeline for Time-Lapse Camera Monitoring of Insects and Their Floral Environments." bioRxiv. https://doi.org/10.1101/2024.04.12.589205

Borji, Ali. 2021. "Pros and Cons of GAN Evaluation Measures: New Developments."

Cameron, Sydney A., Jeffrey D. Lozier, James P. Strange, Jonathan B. Koch, Nils Cordes, Leellen F. Solter, and Terry L. Griswold. 2011. "Patterns of Widespread Decline in North American Bumble Bees." *Proceedings of the National Academy of Sciences* 108(2):662–67. doi:10.1073/pnas.1014743108.

Cao, Kaidi, Colin Wei, Adrien Gaidon, Nikos Arechiga, and Tengyu Ma. 2019. "Learning Imbalanced Datasets with Label-Distribution-Aware Margin Loss."

Cui, Yin, Menglin Jia, Tsung-Yi Lin, Yang Song, and Serge Belongie. 2019. "Class-Balanced Loss Based on Effective Number of Samples." Pp. 9260–69.

Deng, Jia, Wei Dong, Richard Socher, Li-Jia Li, Kai Li, and Li Fei-Fei. 2009. "ImageNet: A Large-Scale Hierarchical Image Database." Pp. 248–55 in *2009 IEEE Conference on Computer Vision and Pattern Recognition*.

GBIF.org. 2025. *GBIF Home Page*. Available from: https://www.gbif.org [accessed 12 March 2026].

Ghiasi, Golnaz, Yin Cui, Aravind Srinivas, Rui Qian, Tsung-Yi Lin, Ekin D. Cubuk, Quoc V. Le, and Barret Zoph. 2021. "Simple Copy-Paste Is a Strong Data Augmentation Method for Instance Segmentation."

Goulson, Dave. 2010. *Bumblebees: Behaviour, Ecology, and Conservation*. Oxford: Oxford University Press.

He, Kaiming, Xiangyu Zhang, Shaoqing Ren, and Jian Sun. 2015. "Deep Residual Learning for Image Recognition."

He, Ruifei, Shuyang Sun, Xin Yu, Chuhui Xue, Wenqing Zhang, Philip Torr, Song Bai, and Xiaojuan Qi. 2023. "Is Synthetic Data from Generative Models Ready for Image Recognition?"

Heusel, Martin, Hubert Ramsauer, Thomas Unterthiner, Bernhard Nessler, and Sepp Hochreiter. 2018. "GANs Trained by a Two Time-Scale Update Rule Converge to a Local Nash Equilibrium."

Horn, Grant Van, Oisin Mac Aodha, Yang Song, Yin Cui, Chen Sun, Alex Shepard, Hartwig Adam, Pietro Perona, and Serge Belongie. 2018. "The iNaturalist Species Classification and Detection Dataset."

Huh, Minyoung, Brian Cheung, Tongzhou Wang, and Phillip Isola. 2024. "The Platonic Representation Hypothesis." ICML 2024.

Jacobson, Molly M., Erika M. Tucker, Minna E. Mathiasson, and Sandra M. Rehan. 2018. "Decline of Bumble Bees in Northeastern North America, with Special Focus on Bombus Terricola." *Biological Conservation* 217:437–45. doi:10.1016/j.biocon.2017.11.026.

Jayasumana, Sadeep, Srikumar Ramalingam, Andreas Veit, Daniel Glasner, Ayan Chakrabarti, and Sanjiv Kumar. 2024. "Rethinking FID: Towards a Better Evaluation Metric for Image Generation."

Kang, Bingyi, Saining Xie, Marcus Rohrbach, Zhicheng Yan, Albert Gordo, Jiashi Feng, and Yannis Kalantidis. 2020. "Decoupling Representation and Classifier for Long-Tailed Recognition."
...
Kornblith, Simon, Jonathon Shlens, and Quoc V. Le. 2019. "Do Better ImageNet Models Transfer Better?" Pp. 2656–66 in *2019 IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR)*.

Liang, Yijun, Shweta Bhardwaj, and Tianyi Zhou. 2025. "Diffusion Curriculum: Synthetic-to-Real Data Curriculum via Image-Guided Diffusion."

Lin, Tsung-Yi, Priya Goyal, Ross Girshick, Kaiming He, and Piotr Dollár. 2018. "Focal Loss for Dense Object Detection."

Lin, Tsung-Yu, Aruni RoyChowdhury, and Subhransu Maji. 2018. "Bilinear Convolutional Neural Networks for Fine-Grained Visual Recognition." *IEEE Transactions on Pattern Analysis and Machine Intelligence* 40(06):1309–22. doi:10.1109/TPAMI.2017.2723400.

Liu, Ziwei, Zhongqi Miao, Xiaohang Zhan, Jiayun Wang, Boqing Gong, and Stella X. Yu. 2019. "Large-Scale Long-Tailed Recognition in an Open World." https://arxiv.org/abs/1904.05160v2.

Lu, Yujie, Xianjun Yang, Xiujun Li, Xin Eric Wang, and William Yang Wang. 2023. "LLMScore: Unveiling the Power of Large Language Models in Text-to-Image Synthesis Evaluation."

MacKenzie, Darryl I., James D. Nichols, Gideon B. Lachman, Sam Droege, J. Andrew Royle, and Catherine A. Langtimm. 2002. "Estimating Site Occupancy Rates When Detection Probabilities Are Less Than One." *Ecology* 83(8):2248–55. doi:10.1890/0012-9658(2002)083[2248:ESORWD]2.0.CO;2.

MacPhail, Victoria J., Richard Hatfield, and Sheila R. Colla. 2024. "Bumble Bee Watch Community Science Program Increases Scientific Understanding of an Important Pollinator Group across Canada and the USA." *PLOS ONE* 19(5):e0303335. doi:10.1371/journal.pone.0303335.

Michaeli, Eyal, and Ohad Fried. 2025. "Advancing Fine-Grained Classification by Structure and Subject Preserving Augmentation."

Monsefi, Amin Karimi, Mridul Khurana, Rajiv Ramnath, Anuj Karpatne, Wei-Lun Chao, and Cheng Zhang. 2025. "TaxaDiffusion: Progressively Trained Diffusion Model for Fine-Grained Species Generation."
...
Otani, Mayu, Riku Togashi, Yu Sawai, Ryosuke Ishigami, Yuta Nakashima, Esa Rahtu, Janne Heikkilä, and Shin'ichi Satoh. 2023. "Toward Verifiable and Reproducible Human Evaluation for Text-to-Image Generation."

Plath, Otto Emil. 1934. *Bumblebees and Their Ways*. New York: The Macmillan Company.
...
Richardson, Leif L., Kent P. McFarland, Sara Zahendra, and Spencer Hardy. 2019. "Bumble Bee (Bombus) Distribution and Diversity in Vermont, USA: A Century of Change." *Journal of Insect Conservation* 23(1):45–62. doi:10.1007/s10841-018-0113-5.

Richardson, Leif, and Sheila Colla. 2025. "Bumble Bees of the Eastern United States." https://www.academia.edu/12139962/Bumble_bees_of_the_Eastern_United_States.

Salimans, Tim, Ian Goodfellow, Wojciech Zaremba, Vicki Cheung, Alec Radford, and Xi Chen. 2016. "Improved Techniques for Training GANs."

Schilthuizen, Menno. 2025. *The Urban Naturalist: How to Make the City Your Scientific Playground*. Cambridge, MA: MIT Press.

Shipard, Jordan, Arnold Wiliem, Kien Nguyen Thanh, Wei Xiang, and Clinton Fookes. 2023. "Diversity Is Definitely Needed: Improving Model-Agnostic Zero-Shot Classification via Stable Diffusion." CVPR Workshops 2023.

Shorten, Connor, and Taghi M. Khoshgoftaar. 2019. "A Survey on Image Data Augmentation for Deep Learning." *Journal of Big Data* 6(1):60. doi:10.1186/s40537-019-0197-0.

Spiesman, Brian J., Claudio Gratton, Richard G. Hatfield, William H. Hsu, Sarina Jepsen, Brian McCornack, Krushi Patel, and Guanghui Wang. 2021. "Assessing the Potential for Deep Learning and Computer Vision to Identify Bumble Bee Species from Images." *Scientific Reports* 11(1):7580. doi:10.1038/s41598-021-87210-1.

Stevens, Samuel, Jiaman Wu, Matthew J. Thompson, Elizabeth G. Campolongo, Chan Hee Song, David Edward Carlyn, Li Dong, Wasila M. Dahdul, Charles Stewart, Tanya Berger-Wolf, Wei-Lun Chao, and Yu Su. 2024. "BioCLIP: A Vision Foundation Model for the Tree of Life."

The Beecology Project. n.d. Retrieved March 12, 2026. https://beecology.wpi.edu/.

Venverloo, Titus, and Fábio Duarte. 2024. "Towards Real-Time Monitoring of Insect Species Populations." *Scientific Reports* 14(1):18727. doi:10.1038/s41598-024-68502-8.

Wang, Xudong, Long Lian, Zhongqi Miao, Ziwei Liu, and Stella X. Yu. 2022. "Long-Tailed Recognition by Routing Diverse Distribution-Aware Experts."

Wei, Xiu-Shen, Yi-Zhe Song, Oisin Mac Aodha, Jianxin Wu, Yuxin Peng, Jinhui Tang, Jian Yang, and Serge Belongie. 2021. "Fine-Grained Image Analysis with Deep Learning: A Survey."

Williams, Paul, Robbin Thorp, Leif Richardson, and Sheila Colla. 2014. *Bumble Bees of North America: An Identification Guide*.

Xu, Jiazheng, Xiao Liu, Yuchen Wu, Yuxuan Tong, Qinkai Li, Ming Ding, Jie Tang, and Yuxiao Dong. 2023. "ImageReward: Learning and Evaluating Human Preferences for Text-to-Image Generation."

Yang, Chih-Hsuan, Ben Feuer, Zaki Jubery, Zi K. Deng, Andre Nakkab, Zahid Hasan, Shivani Chiranjeevi, Kelly Marshall, Nirmal Baishnab, Asheesh K. Singh, Arti Singh, Soumik Sarkar, Nirav Merchant, Chinmay Hegde, and Baskar Ganapathysubramanian. 2024. "BioTrove: A Large Curated Image Dataset Enabling AI for Biodiversity." *Advances in Neural Information Processing Systems* 37:102101–20. doi:10.52202/079017-3241.

Yu, K., et al. 2025. "GPT-ImgEval: A Comprehensive Benchmark for Diagnosing GPT4o in Image Generation."

Zhao, Qihao, Yalun Dai, Hao Li, Wei Hu, Fan Zhang, and Jun Liu. 2024. "LTGC: Long-Tail Recognition via Leveraging LLMs-Driven Generated Content."

Zheng, Heliang, Jianlong Fu, Tao Mei, and Jiebo Luo. 2017. "Learning Multi-Attention Convolutional Neural Network for Fine-Grained Image Recognition." Pp. 5219–27.
