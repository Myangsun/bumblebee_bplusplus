...
#### 5.3.4 Judge reliability: viewpoint bias and rubric refinement validation

The contact-sheet grids produced from the v1 judge output
(`RESULTS_kfold/llm_judge_eval/contact_Bombus_<species>.png`) surface a
systematic pattern: frontal / head-on poses of the rare species are
over-represented in the FAIL panel of each contact sheet, disproportionately
to any other view angle. On the full 500-image B. ashtoni pool, 33 of 102
frontal shots (32 %) fall into the soft_fail tier (`matches_target = True`
but `diagnostic_completeness ≠ "species"`), against 4–19 % across the other
four view-angle bins. B. sandersoni shows a more extreme form — frontal
soft_fail rate 26 %, every other angle 0 %. B. flavidus shows the most
extreme: frontal soft_fail rate 89 % (89 of 100 frontal shots), other angles
18–29 %. This is a consistent per-species pattern that a pose-agnostic
filter cannot explain away as random noise.

Prompt-engineering inspection of `scripts/llm_judge.py` identifies two
problems in the v1 rubric that jointly produce this pattern. First, the
judge's `not_visible: bool` flag is defined as "obscured or not visible"
but the rubric exposes no third state for *in frame but not
diagnostically assessable from this viewpoint*. On frontal shots of the
rare species the dorsal tergite pattern — the primary species-diagnostic
surface — is not in view; the judge can either mark `not_visible = True`
(correct on 21 % of frontal ashtoni shots) or assign a low-confidence
guess score which drags `mean_morph` down and propagates into a `"genus"`
verdict on `diagnostic_completeness` (soft_fail). Second, identical
re-runs of the v1 judge at `temperature = 0` produce different verdicts on
the same image at a high rate, independent of any rubric issue: on the
validation cohort below, 21 % of B. ashtoni `overall_pass` labels and
27 % of the `diagnostic_completeness` levels flip between two identical
API calls. The v1 output layer is therefore noisy enough that per-image
claims drawn from a single v1 run carry a measurable reliability
penalty before any calibration-gap analysis is applied.

A rubric refinement (`scripts/llm_judge_decomposed.py`) addresses both
problems with a single schema change: the binary `not_visible` flag is
replaced with a tri-state `visibility ∈ {visible, not_assessable,
not_visible}`, where `not_assessable` names the viewpoint-limited state
directly and excludes the feature from the `mean_morph` computation. The
pass rule stays v1-shaped — `matches_target ∧ diag ∈ {species, genus}
∧ mean_morph ≥ 4.0 ∧ no repetitive_pattern` — but `mean_morph` now
averages only over features the judge actually graded. All anchors and
thresholds are pre-committed against the v1 scale (not tuned on expert
labels).

Validation ran on a stratified 80-image cohort per rare species (40 v1
soft_fails + 40 v1 strict_passes; B. sandersoni has only 26 soft_fails
in the 500-image pool, so its cohort is 66 images), under four
configurations: v1 run 1 (the original `results.json`), v1 run 2 (fresh
API call, identical prompt), v2 tri-state run 1, v2 tri-state run 2.
All runs use GPT-4o at `temperature = 0` with structured Pydantic
output. Three findings survive multiple-species replication
(N = 226 total images).

![Test-retest stability: v1 fused vs v2 tri-state, three rare species](plots/judge_decomposed/stability_3species.png)
*Figure 5.11a: v1 (fused binary visibility) versus v2 (tri-state visibility) test-retest stability on the same 226 images. Top row: `overall_pass` and `diagnostic_completeness` flip rates between identical re-runs. Bottom left: v2 `mean_morph` drift from run 1 to run 2 per image — tight dispersion around the diagonal. Bottom right: tri-state usage frequency per species × feature, showing v2 is actually using the new states rather than defaulting to `visible`.*

**Finding 1 — v1 is measurably unstable at T = 0.** Across all three
rare species, v1 flips 8.8 % of `overall_pass` verdicts (21 % on B.
ashtoni, where the viewpoint pattern is strongest) and 26 % of
`diagnostic_completeness` levels on identical re-runs. This noise sits
underneath every single-run v1 claim in §5.3 and the 150-label
agreement statistics in §5.4: the LLM-vs-expert strict-pass
agreement of 56 % (§5.4.2) and morph-mean AUC of 0.56 include ~20
percentage points of v1 self-noise as a lower bound.

**Finding 2 — the tri-state rubric is roughly 3× more stable.** v2 flips
`overall_pass` on 3.5 % of images overall (v1: 8.8 %) and
`diagnostic_completeness` on 5.8 % (v1: 26 %). The `diag`-flip
improvement is the mechanistically load-bearing one because `diag` is
the field that drives `soft_fail` and therefore the contact-sheet FAIL
panel. v2 mean_morph run-to-run deviation is 0.14–0.27 points on the 1–5
scale, with 99 % of images within ±1 between runs.

![Viewpoint resolution: v1 soft_fail by pose vs v2 verdict + tri-state usage by pose](plots/judge_decomposed/viewpoint_resolution_3species.png)
*Figure 5.11b: Does v2 tri-state resolve the frontal-shot pattern that dominated the v1 contact-sheet FAIL panels? Left column: v1 soft_fail rate by view-angle on the full 500-image pool per species — the raw problem. Right column: v2 verdict (stacked PASS vs FAIL) and abdomen-skip rate by view-angle on the 80-image validation cohort. On frontal shots, v2 correctly marks abdomen_banding `not_assessable` or `not_visible` 83 %–100 % of the time per species, and the resulting verdict is PASS on 95 %–100 % of the same cohort frontals.*

**Finding 3 — the tri-state flag is used meaningfully, and
species-specifically.** v2 invokes `not_assessable` at rates that match
each species' pose-diagnostic-surface geometry (Figure 5.11a, lower
right). B. ashtoni reaches 16 % `not_assessable` on abdomen_banding and
15 % `not_visible` on head_antennae (the frontal cluster); B. sandersoni
reaches 46 % `not_assessable` on abdomen_banding because the diagnostic
T4 white tip needs a dorsal view; B. flavidus reaches 28 %
`not_assessable` on head_antennae. These are the judgements v1 either
defaulted to `not_visible = False` and guessed on, or partially flipped
between runs.

The validation is deliberately scoped to demonstrate the rubric
mechanism rather than to re-filter the 1,500-image pool. v2 has not been
run end-to-end on the full generation pool, no D4′ training partition
has been built, and the 150-label expert-agreement AUC has not been
re-measured under v2 — those extensions are registered in §7.2.1. The
material consequence for this thesis is a reliability caveat on every
single-run v1 claim in §5.3 and §5.4: the 56 % strict-pass agreement and
0.56 morph-mean AUC of §5.4.2 are lower bounds on what a rubric-refined
LLM judge can achieve, and the per-image D4 vs D6 contact-grid
comparisons of §5.4.4 carry a v1 noise floor that the expert probe (D6),
operating on BioCLIP features directly, does not inherit.
...
Prediction (iii) is **confirmed at seed 42**: Table 5.7's six D6 cells are all non-harmful (−0.121 / −0.056 / −0.025 own-species, no positive off-diagonal), and D6 is the only variant in the table with this property. D5 is intermediate — it still retains the D3 harm pattern on B. ashtoni (+0.182) and now B. sandersoni (+0.056), indicating that unsupervised centroid distance does not suffice to filter out the harmful subset. Predictions (i) and (ii) are **confirmed at the per-image level in the now-complete D5 and D6 chain galleries** (Appendix F, `chains_d2_centroid_*` and `chains_d6_probe_*` on disk). D6 produces zero harmed chains for B. ashtoni and B. sandersoni — the per-image form of the Table 5.7 "no harmful cells" result — while D5 retains D3-style harmed-chain clusters for both species, consistent with unsupervised centroid distance failing to filter the harmful subset. Under the single-seed ablation reading, the expert-supervised filter resolves the LLM miscalibration rather than merely relocating it — the prediction of Section 5.4.5 is supported in direction, with statistical significance deferred (Section 6.5) because the fold-level paired t-test at df = 4 cannot detect the ≤ 0.01 macro-F1 gap D6 produces over D3 / D4 (Table 5.6).
...
A reliability caveat further widens the gap. Section 5.3.4 shows that the v1 judge flips `overall_pass` on 9 % of images (21 % on B. ashtoni) and `diagnostic_completeness` on 26 % of images between identical `temperature = 0` re-runs, tracing a sizable fraction of the calibration gap to rubric architecture rather than model misjudgement. A rubric-refined v2 (tri-state visibility) is roughly 3× more stable on the same 226-image validation cohort and correctly handles 95–100 % of frontal shots that v1 pushed into soft_fail. The 56 % strict-pass agreement and the 0.56 morph-mean AUC reported above are therefore *lower bounds* on what a language-mediated rubric can achieve on this task; the D4 vs. D6 gap at the per-image contact-grid level carries a v1 noise floor that the expert probe (D6, operating directly on BioCLIP features) does not inherit. The rubric refinement is scoped as a methodology validation, not a D4′ replacement (§7.2.1).
